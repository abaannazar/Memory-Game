...
const initializeGame = () => {
    ...
    const numbers = [...Array(pairCount).keys()].map((n)=>n+1); //[1,2,3,4,5,6,7,8]
    ...
    <div className='flex flex-col items-center justify-center min-h-screen bg-grey-100 p-4'>
        ...
        style={{
          ...
          {cards.map((card) => {
            ...
            className={`aspect-square flex items-center justify-center text-1xl 
            font-bold border rounded-lg cursor-pointer transition-all 
            duration-300 ${
              isFlipped(card.id) 
              ? isSolved(card.id)
